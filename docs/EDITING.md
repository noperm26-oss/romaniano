...
- `buildings/catalog.js` defines Romanian cottage/farm/workshop/chapel sizes and materials.
- `buildings/batches.js` creates their visual instances and **matching AABB collision footprints**.
- `world/settlements.js` places spaced homesteads, streets, orchards and crop rows throughout the territory. Existing towns/landmarks have protected margins.
- Existing walk-in interiors stay in `buildings/interiors.js`; towns, landmarks and villages each have their own component.
- `world/districts.js` retains the original economic districts, replacing overlapping scatter with spaced blocks and chunked rendering.

Keep roads, doorways and unit clearance open. A new rotated solid needs a matching conservative collider (not an unrotated footprint). Call `addCollider()` so navigation caches invalidate. Do not write directly to the collider array. Decorative greenery/crops are intentionally non-solid; solid houses always register footprints. New homestead houses are exterior scenery, not walk-in interiors.

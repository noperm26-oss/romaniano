...
- `buildings/architecture.js` — `kitCreate()` (merged geometry with a local frame; `kit.at()`+`kit.quad/tri` for hand-built polygons, `kit.exportGeo()` for instancing), `structShell(o)` (walls with **real door and window openings**, shutters, glass, porch or full-length `prispa`, `bench`, plinth and foundation skirt, roofs `gable|long|thatch|hip|hipthatch|flat|hall|church` with `gableWin`, second storey, tower, dome, arcade, buttresses, battlements, log courses, visible half-timbering (`timberFrame`/`braces`), the painted `band`, shop signs), `spawnDoorLeaf()` and `wallColliders()`.
- `buildings/prefabs.js` — **the countryside tier**. A prefab is any builder run once at the origin under `prefabCapture()` (which sets `PREFAB_REC`, so `addCollider`, `regLight`, `regChimney`, `regStructure`, `spawnDoorLeaf` and `addFlame` record instead of acting) and turned into a vertex-coloured body geometry + a `PANE_MAT` pane geometry. `prefabPlace(key,x,z,face,{force,maxSlope,y,name})` stamps an instance facing `S|E|N|W` (quarter turns only — colliders stay axis-aligned), re-adds its colliders, an instanced door leaf in `DOORS`, lights, chimney and an enterable `STRUCTURES` record; `prefabFlush()` (boot, after settlements) builds one `InstancedMesh` per prefab per 375 u cell inside the culled `propCell` groups. House keys are `region.type+variant` (`moldavian.cottage2`, `capital.house1`, `wallachian.barn1`, …) generated from `PREFAB_PAL` by `prefabHouseDef()`; yard pieces are `prop.woodpile|haystack|oven|beehives|dovecote|well|cart|troita|tent`. To add a prefab: add a `PREFAB_DEFS['prop.x']=function(kit,gy){…}` builder (world coordinates around 0,0 at height `gy`, colliders via `addCollider`) or a new type in `prefabHouseDef()`; never call `groundH` for anything but `gy`, and keep interiors free of `propAdd`/`ANIM_PARTS` (they would be discarded).
...
- `world/roads.js` — draped road ribbons per class (kerbs, colours, +speed), milestones, junction crosses/signposts, wells, gate lanterns, toll arches (`window.__roadPts`, `roadStats`, `ROAD_JUNCTIONS`, `roadNearest()/offRoad()` in areas.js keep props off the carriageway); `world/districts.js` — farmsteads/folds/quarries/camps (`DISTRICT_KITS`: prefab houses ringing the well, facing it), the zone economy (`SITES`, `zoneSiteRate`) and the wilds pass; `world/settlements.js` — `buildHamlet()` for every `HAMLETS` site (geography.js levels the ground): lane coordinates (u along, v across, `alongX` per hamlet), `HAMLET_KINDS[region]` house mix, yards (`hamletYard`: fences with a gate gap, path, garden, shed, oven, woodpile, beehives, dovecote, trees), lane-end chapel/barn, well + troiță + cart, fields, orchards; `buildings/batches.js` — `createBuildingBatch()` instanced boxes/trees/crowns for fences, garden rows and fields only (no building is a batch primitive any more); `world/settlements.js` + `buildings/batches.js` — instanced hamlet houses with doors, windows and footprints.
